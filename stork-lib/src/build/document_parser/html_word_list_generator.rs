...
pub(crate) fn generate(
    ...
    if let Ok(document_matches) = document.select(selector) {
        ...
        if matches_vec.is_empty() {
            return Err(DocumentReadError::SelectorNotPresent(selector.to_string()));
        }
            ...
            for elem in as_node.traverse_inclusive() {
                if let kuchiki::iter::NodeEdge::Start(node_ref) = elem {
                    ...
                    if !contents.is_empty() {
                        document_contents.push_str(&contents);
                        let words = segment_words(&contents);
        ...
        if word_list.is_empty() {
            return Err(DocumentReadError::EmptyWordList);
        }

        return Ok((document_contents, word_list));
    }

    Err(DocumentReadError::SelectorNotPresent(selector.to_string()))
}
