...
for config in configs:
    input  = os.path.join(os.getcwd(), "local-dev", "test-configs", f"{config}.toml")
    output = os.path.join(os.getcwd(), "local-dev", "test-indexes", f"{config}.st")

    if os.path.exists(output):
        print(f"local-dev/test-indexes/{config}.st already exists, skipping")
        continue
    
    if not os.path.exists(input):
        print(f"local-dev/test-configs/{config}.toml doesn't exist, skipping")
