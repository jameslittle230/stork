...
impl TryFrom<&str> for StemmingConfig {
...
impl<'de> serde::Deserialize<'de> for StemmingConfig {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if let Ok(Value::String(string)) = Deserialize::deserialize(deserializer) {
            StemmingConfig::try_from(string.as_ref()).map_err(|_e| {
...
mod tests {
    ...
    fn test_none_lowercase() {
        assert_eq!(
            StemmingConfig::try_from("none").unwrap(),
            StemmingConfig::None
        );
    }
    ...
    fn test_dutch() {
        assert_eq!(
            StemmingConfig::try_from("Dutch").unwrap(),
    ...
    #[test]
    fn test_error() {
        assert!(StemmingConfig::try_from("Blorp").is_err())
    }
    ...
    fn test_none_tostring() {
        assert_eq!(String::from(StemmingConfig::None), "none".to_string());
    }
}
